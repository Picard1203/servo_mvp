...
## Summary
- 2464 nodes · 4211 edges · 177 communities (138 shown, 39 thin omitted)
- Extraction: 91% EXTRACTED · 9% INFERRED · 0% AMBIGUOUS · INFERRED: 379 edges (avg confidence: 0.56)
- Token cost: 173,602 input · 7,844 output

## Graph Freshness
- Built from commit: `f86d4a4c`
- Run `git rev-parse HEAD` and compare to check if the graph is stale.
- Run `graphify update .` after code changes (no API cost).

## Community Hubs (Navigation)
- Bridge Network Relay (C++)
- Fine Approach Trial Tool
- Saved Position Exceptions
- Frontend API Client
- Servo Repository Contract
- Operator Metrics Tally
- Relay E2E Tests
- Saved Positions Route Tests
- SQLite Saved Position Repo
- SQLite Database & App State
- Virtual Operator Behavior
- Architecture Decision Records
- Project Manifest & Environment
- Client Behavior Check Script
- Open Questions Doc
- Saved Position CRUD Tests
- Diagnostic Log Ring (C++)
- Domain Entities & Servo Abstraction
- Angle & Log Ring Unit Tests
- MCU Log Tests
...
- Logger Stub
- OnTarget Test Sketch
- Servo Register Types (C++)
- Workflows Doc
- Isolation Service
...
- Telemetry Binary Export Tests
- Sketch Entry Point (C++)
- Backlog T-Items
- Domain Exception Base Classes
- Servo Status & Faults (C++)
- State & Diagnostics Dependencies
- Rig Testing Protocol Doc
- Native Test Framework (C++)
- Relay Connection Ceiling
- Early Defects List
- Servo Bus Draining
- Servo Router Endpoints
- System Audit Doc
- Soak Test Checkpointer
- Simulated Servo Tests
- Hardware Config Fallback
- Motion Command Methods
- Soak Report Tests
- Binary Telemetry Export
- Telemetry Repository
- Motion Service
- Synthetic Operator Tests
- Angle Converter (C++)
- Backlog Items D28-46
- Servo Bus (C++)
- Backlog R-Items
- Sprint Session Batches
- Database Schema Migration
- Hardware Bench Facts
- Firmware Prose Strip Task
- Bridge Stub Test Double
- Telemetry Export Tests
- Twin-Review Skill Doc
- Python Dependencies
- Python Prose Strip Task
- Flaky Servo Test Double
- Logging Setup
- Saved Position Repository Contract
- Calibration Service Tests
- Bridge Diagnostic Reads
- Design Diagrams Task
- MCU Log Receiver
- E2E Backend Fixtures
- Fault Bits & Sign-Magnitude Tests
- SPI Remap (C++)
- Deploy Push Notes
...
- Closed Sessions Doc
- MCU Log Dependency Tests
- Domain Glossary
- Sprint Planning Log
- Small Screen UI Bug
- Servo Domain Exceptions
...
- Serial Monitor Note
- Command Acknowledgement Tests
- Saved Positions Routes
- Brace Balance Checker Script
- Saved Positions Datum
- Failed Read Guard Tests
- Angle-Counts Conversion
- App Config And Relay
- Servo Routes Tests
- Bridge API (C++)
- Servo State Store Tests
- Repo Working Conventions
- Calibration And Saved Positions
- Sign-Magnitude Decoding
- Sketch Class Design
- Motion/State Dependency Providers
- Simulated Path Coverage Caveat
- Soak Test Runner
- Position Terminology
- Mechanical Restraint Lock
- Servo Snapshot Reading
- Apply Conventions Task
- Repository Layout
- Relay Error Paths
- Bridge Fault Resilience
- Snapshot Decoding Tests
- Sign-Magnitude Encoding
- ST3215 Hardware Skill Doc
- Relay Failure Paths
- Backend Selection Tests
- Invalid Flag Handling
- Motion Service Fixture
- Settle State Reading
- Saved Position Fixture
...
- `stream()` --references--> `_get()`  [EXTRACTED]
  python/app/routers/stream.py → tools/fine_approach_trial.py
...
## Communities (177 total, 39 thin omitted)

### Community 0 - "Bridge Network Relay (C++)"
Cohesion: 0.10
Nodes (24): k_timeout_t, BridgeApi::BridgeApi(), ByteSink, CloseSink, OpenSink, ByteSink, CloseSink, OpenSink (+16 more)

### Community 1 - "Fine Approach Trial Tool"
Cohesion: 0.12
Nodes (27): _append_csv(), _csv_path(), _latest_event(), main(), _median(), _move(), _now_iso(), _parse_targets() (+19 more)

### Community 2 - "Saved Position Exceptions"
Cohesion: 0.09
Nodes (22): DuplicateNameError, NotFoundError, PositionOutOfRangeError, Raised when a referenced entity does not exist., Raised when a saved position's angle falls outside the travel window., Raised when a saved position name is already in use., Raised when an edit targets a saved position changed since it was read., StalePositionError (+14 more)
...
### Community 4 - "Servo Repository Contract"
Cohesion: 0.03
Nodes (37): Contract for reading and commanding the servo (real or simulated)., Reads the present-speed register directly. Returns: Optional[int]: Signed…, Returns the full instantaneous sensory readout. Returns: TelemetrySnapshot:…, Starts a move toward an absolute counts target. Args: target_counts (int):…, Stops motion at the current position. Returns: bool: True when the servo…, Configures the travel-range mode before normal operation. Args: multi_turn…, Configures the servo dead-zone width. Args: counts (int): Dead-zone width in…, Cuts or restores drive torque while sensors stay powered. Args: enabled (bool):… (+29 more)
...
### Community 8 - "SQLite Saved Position Repo"
Cohesion: 0.12
Nodes (18): A named, described position an operator can return to. Attributes: id…, SavedPosition, SQLite implementation of the saved-position repository., Deletes one saved position. Args: position_id (int): Database identifier.…, Stores saved positions in the saved_positions table. Attributes: _db…, Maps a database row to the entity. Args: row (object): SQLite row. Returns:…, Persists a new saved position. Args: position (SavedPosition): Entity with…, Returns all saved positions, newest first. Returns: list[SavedPosition]: All… (+10 more)

### Community 9 - "SQLite Database & App State"
Cohesion: 0.08
...
### Community 15 - "Saved Position CRUD Tests"
Cohesion: 0.19
...
### Community 16 - "Diagnostic Log Ring (C++)"
Cohesion: 0.10
Nodes (20): kLogRingCapacity, DiagLog, Drain, dropped_total, Init, lock_, Push, ring_ (+12 more)

### Community 17 - "Domain Entities & Servo Abstraction"
Cohesion: 0.11
Nodes (14): Immutable domain entities shared across layers., Diagnostic read of the servo's position-loop tuning registers. Attributes:…, TuningRegisters, ABC, Abstract servo access: the seam between simulation and hardware., Reads the position-loop tuning registers directly. Returns:…, ABC, Abstract persistence of telemetry samples. (+6 more)

### Community 18 - "Angle & Log Ring Unit Tests"
Cohesion: 0.07
...
### Community 19 - "MCU Log Tests"
Cohesion: 0.19
Nodes (7): _lines(), McuLog: receiving and writing diagnostic events forwarded from the MCU., Reads every JSON line from a file. Args: path: File to read. Returns: One dict…, One event in, one JSON line out., Size-based single-backup rotation., TestRotation, TestWriting

### Community 20 - "Isolation Gate Tests"
Cohesion: 0.25
Nodes (4): The two gates must not collapse into one reason code - an operator refused for…, Neither single-condition gate may fire alone here - an operator who clears one…, Motor isolation gates a move the same way the digital lock does., TestIsolationGate
...
### Community 22 - "Settings Config Tests"
Cohesion: 0.17
...
### Community 23 - "Events & Composition Root"
Cohesion: 0.07
Nodes (31): EventService, In-memory ring buffer of structured events for the events endpoint., Thread-safe fixed-size store of recent events. Attributes: _events…, InvalidReadingError, Raised when an operation needs a reading the servo did not supply., Composition root: cached provider functions that construct and wire., Calibration, The calibration datum. Attributes: raw_counts (int): Absolute encoder position… (+23 more)
...
### Community 25 - "Fake Bridge Fixture"
Cohesion: 0.20
...
### Community 26 - "Servo Controller (C++)"
Cohesion: 0.27
Nodes (17): ClampAmplification(), ClampByteRegister(), ClampDeadband(), ClampMinStartForce(), ServoController, Begin, CentreHere, ClearFault (+9 more)
...
### Community 33 - "OnTarget Test Sketch"
Cohesion: 0.48
Nodes (5): Check(), CheckNear(), MoveTo(), setup(), WaitSettled()

### Community 34 - "Servo Register Types (C++)"
Cohesion: 0.12
Nodes (13): ReadTuningRegisters, MoveCommand, acceleration, speed_counts_per_second, target_counts, TuningSnapshot, ccw_dead_zone, cw_dead_zone (+5 more)
...
### Community 36 - "Isolation Service"
Cohesion: 0.11
Nodes (15): get_event_service(), get_isolation_service(), Returns the motor-isolation service. Returns: IsolationService: The process-…, Returns the shared event buffer. Returns: EventService: The process-wide event…, Starts the telemetry sampler, isolation reconciler and Bridge relay. Returns:…, _start_background(), IsolationService: intent, reconciliation against hardware, idle backup., The idle timer only ever catches 'locked but forgot to isolate'. (+7 more)

### Community 37 - "Bridge Servo Repository Tests"
Cohesion: 0.20
Nodes (8): BridgeServoRepository, Talks to the servo through the MCU Bridge. Attributes: _bridge (object): Object…, Creates the repository. Args: bridge (Optional[object]): Optional Bridge…, With no bridge injected it falls back to the Arduino Bridge., The Bridge is a single multiplexed link; only one call at a time., Two threads reading at once must not overlap on the wire. Overlapping RPC…, TestConcurrencySafety, TestDefaultBridge

### Community 38 - "App Loop Stub"
Cohesion: 0.50
Nodes (3): AppStub, Stub of the App loop runner., Does nothing. Returns: None.

### Community 39 - "SSE Streaming"
Cohesion: 0.10
Nodes (28): EventDep, ge, le, StreamingResponse, SSE stream for servo state, saved positions and events., Streams servo state, saved positions, and events over SSE. Args: request…, Generates SSE events for state, saved positions and events. Args: request…, stream() (+20 more)
...
### Community 42 - "Telemetry Binary Export Tests"
Cohesion: 0.29
...
### Community 43 - "Sketch Entry Point (C++)"
Cohesion: 0.22
...
### Community 45 - "Domain Exception Base Classes"
Cohesion: 0.18
Nodes (11): ConflictException, NotFoundException, Exception, Domain exceptions: each carries its own HTTP mapping and log metadata., Appends this class's error-code segment to its parent's. Args: code…, Base for 409s: refused given the system's current state., Base for 404s: a referenced entity does not exist., Base for 422s: well-formed but refused on its content. (+3 more)

### Community 46 - "Servo Status & Faults (C++)"
Cohesion: 0.10
Nodes (18): ReadSnapshot, ServoFaults, angle, overcurrent, overheat, overload, sensor, voltage (+10 more)

### Community 47 - "State & Diagnostics Dependencies"
Cohesion: 0.16
Nodes (13): get_present_speed(), get_state(), get_torque_register(), get_tuning_registers(), Reads the servo torque-enable register directly. Args: servo (ServoRepository):…, Reads the servo's position-loop tuning registers directly. Args: servo…, Reads the servo present-speed register directly. Args: servo (ServoRepository):…, Returns the full state snapshot for the client. Args: state (ServoStateStore):… (+5 more)
...
### Community 49 - "Native Test Framework (C++)"
Cohesion: 0.16
...
### Community 53 - "Servo Router Endpoints"
Cohesion: 0.08
...
### Community 56 - "Simulated Servo Tests"
Cohesion: 0.05
...
### Community 58 - "Motion Command Methods"
Cohesion: 0.17
Nodes (6): Starts a move toward an absolute counts target. Args: target_counts (int):…, Stops motion at the current position. Returns: bool: True when the servo…, Configures the servo dead-zone width. Args: counts (int): Dead-zone width in…, Configures single-turn or multi-turn absolute positioning. Args: multi_turn…, Writes any subset of the position-loop tuning registers directly. Args:…, Invokes a Bridge function and logs a non-ok acknowledgement. Args: name (str):…
...
### Community 60 - "Binary Telemetry Export"
Cohesion: 0.33
Nodes (6): alias, export_binary(), Query, StreamingResponse, Exports compact packed binary telemetry data for client-side rendering. Args:…, TelemetryDep

### Community 61 - "Telemetry Repository"
Cohesion: 0.09
...
### Community 72 - "Bridge Stub Test Double"
Cohesion: 0.18
Nodes (7): BridgeStub, Recording stub of the Arduino Bridge., Clears recorded state between tests. Returns: None., Records a provided callback. Args: name: Bridge function name. fn: The…, Records a call and returns the configured result. Args: name: Bridge function…, Bridge callback registration., TestRegistration

### Community 73 - "Telemetry Export Tests"
Cohesion: 0.20
...
### Community 77 - "Flaky Servo Test Double"
Cohesion: 0.25
...
### Community 79 - "Saved Position Repository Contract"
Cohesion: 0.08
Nodes (21): A saved position enriched with its live angle, for display. Attributes: id…, SavedPositionView, ABC, Abstract persistence of saved positions., Contract for storing and editing saved positions., Persists a new saved position. Args: position (SavedPosition): Entity with…, Returns all saved positions, newest first. Returns: list[SavedPosition]: All…, Returns one saved position by id. Args: position_id (int): Database identifier.… (+13 more)

### Community 80 - "Calibration Service Tests"
Cohesion: 0.20
Nodes (7): get_calibration_service(), Returns the calibration service. Returns: CalibrationService: The process-wide…, Calibration, saved positions, state store and motion interacting., TestCalibrationAndSavedPositionsAcrossServices, fixture, Fresh calibration service. Returns: The service under test., service()

### Community 81 - "Bridge Diagnostic Reads"
Cohesion: 0.20
Nodes (5): Cuts or restores drive torque while sensors stay powered. Args: enabled (bool):…, Reads register 0x28 directly. Returns: Optional[int]: Register value (0 or 1),…, Reads the position-loop tuning registers directly. Returns:…, Reads the present-speed register directly. Returns: Optional[int]: Signed…, Invokes a Bridge function, converting failures into empty results. Args: name…

### Community 83 - "MCU Log Receiver"
Cohesion: 0.18
...
### Community 84 - "E2E Backend Fixtures"
Cohesion: 0.19
...
### Community 85 - "Fault Bits & Sign-Magnitude Tests"
Cohesion: 0.20
...
### Community 96 - "MCU Log Dependency Tests"
Cohesion: 0.25
...
### Community 100 - "Servo Domain Exceptions"
Cohesion: 0.09
Nodes (31): CommandNotAcknowledgedError, IsolatedError, LockedAndIsolatedError, LockedError, MovingError, OutOfTravelError, Raised when a target lies outside the servo's physical count range., Raised when a commanded angle violates the configured step size. (+23 more)

### Community 101 - "Repository Provider Functions"
Cohesion: 0.05
Nodes (40): get_database(), get_saved_position_repository(), get_telemetry_repository(), get_telemetry_service(), Returns the telemetry service. Returns: TelemetryService: The process-wide…, Returns the shared database wrapper. Returns: Database: The process-wide…, Returns the saved-position repository. Returns: SavedPositionRepository: The…, Returns the telemetry repository. Returns: TelemetryRepository: The process-… (+32 more)
...
### Community 128 - "Saved Positions Routes"
Cohesion: 0.11
...
### Community 131 - "Failed Read Guard Tests"
Cohesion: 0.13
...
### Community 132 - "Angle-Counts Conversion"
Cohesion: 0.29
...
### Community 133 - "App Config And Relay"
Cohesion: 0.33
Nodes (3): Typed application settings loaded from the environment / .env file., Linux half of the TCP relay; counterpart of EthernetRelay (sketch)., Receives diagnostic events forwarded from the MCU side.

### Community 134 - "Servo Routes Tests"
Cohesion: 0.05
Nodes (29): get_app_state_repository(), Returns the persisted operator-intent repository. Returns: AppStateRepository:…, Instantaneous sensory readout from the servo layer. Attributes: raw_counts…, TelemetrySnapshot, Servo API routes: state, move, stop, lock, calibrate, recover., POST /api/v1/servo/calibrate., POST /api/v1/servo/recover., guard_move_to_lock surfaces as 409 reason=moving. (+21 more)

### Community 135 - "Bridge API (C++)"
Cohesion: 0.21
Nodes (22): bin_t, Ack(), BridgeApi, DrainDiagLog, FormatSnapshot, Register, FieldAt(), ForwardDiagLog() (+14 more)

### Community 136 - "Servo State Store Tests"
Cohesion: 0.08
Nodes (14): ServoStateStore: conversions, lock/settle, verified flag, snapshot., Coherent snapshot content., The display and the motion path must share one baseline. Observed on the board:…, servo_direction inverts commanded and reported motion together. Round-trip…, Target angle: set on accept, staleness, never a fabricated 0.0., Stale, not cleared: 'asked for 45, stopped at 27' is the reading the target…, Lock state and settle window., Post-boot position verification. (+6 more)

### Community 139 - "Sign-Magnitude Decoding"
Cohesion: 0.21
Nodes (8): decode_sign_magnitude(), Servo access through the Bridge to the MCU., Decodes a sign-magnitude field from the servo wire format. Args: value (int):…, BridgeServoRepository: the CSV contract with the sketch. No board and no Bridge…, parametrize, Sign-magnitude decoding: the ~32700 wrap bug., Wire-format decoding for STS position fields., TestDecodeSignMagnitude

### Community 141 - "Motion/State Dependency Providers"
Cohesion: 0.11
Nodes (22): get_motion_service(), get_saved_position_service(), get_state_store(), Returns the atomic servo/lock/baseline/isolation state store. Returns:…, Returns the motion service. Returns: MotionService: The process-wide motion…, Returns the saved-position service. Returns: SavedPositionService: The process-…, Polls a predicate until true or timeout. Args: predicate: Zero-argument…, wait_until() (+14 more)
...
### Community 146 - "Servo Snapshot Reading"
Cohesion: 0.33
Nodes (3): Builds the reading used when the bus did not answer. Returns:…, Reads one coherent snapshot from the servo. Returns: TelemetrySnapshot: The…, Performs one real bus read. Returns: TelemetrySnapshot: The snapshot or empty…

### Community 153 - "ST3215 Hardware Skill Doc"
Cohesion: 0.11
Nodes (18): 1. This is not a normal Arduino, 2. The ST3215 servo, 3. Geometry — and the one law that matters, 4. The Ethernet-shield relay, 5. The Bridge contract, 6. Symptom → cause, 7. Deployment traps, 8. Working rules (+10 more)

### Community 157 - "Motion Service Fixture"
Cohesion: 0.67
Nodes (3): motion(), fixture, Fresh motion service. Returns: The service under test.

### Community 159 - "Saved Position Fixture"
Cohesion: 0.67
Nodes (3): fixture, Fresh saved-position service. Returns: The service under test., service()
...
### Community 309 - "Operator Event Log"
Cohesion: 0.29
...
### Community 344 - "Bridge Relay Tests"
Cohesion: 0.14
Nodes (10): echo_server(), fixture, BridgeRelay: connection mirroring, byte pumping, teardown paths., Local TCP server standing in for FastAPI; echoes received bytes back prefixed…, Fresh registered relay. Returns: The relay under test., Bridge callback registration., Bytes both directions., relay() (+2 more)

### Community 351 - "Dev Runner & Settings"
Cohesion: 0.07
Nodes (27): get_settings(), Returns the process-wide settings singleton. Returns: Settings: The cached…, get_servo_repository(), Returns the servo repository chosen by use_hardware_servo. Returns:…, _ensure_logger461(), main(), Dev-PC runner: the full backend + web UI, no board required. The app modules…, Boots the backend + web UI and runs the dev console. Returns: None. (+19 more)

### Community 397 - "FastAPI App Assembly"
Cohesion: 0.13
Nodes (18): FastAPI, create_app(), FastAPI application assembly: routers and domain-error mapping., Creates and configures the FastAPI application. Returns: FastAPI: The…, Maps every domain exception to its HTTP response and log line. Args: app…, _register_error_handlers(), Telemetry endpoints: binary telemetry stream export., _ensure_logger461() (+10 more)

### Community 399 - "System Routes Tests"
Cohesion: 0.12
...
## Knowledge Gaps
- **374 isolated node(s):** `state`, `REFUSALS`, `EVENT_LABELS`, `DAY_SHEET_COLS`, `RAW_HEADERS` (+369 more)
...
- **Why does `get_state_store()` connect `Motion/State Dependency Providers` to `Saved Position Exceptions`, `Failed Read Guard Tests`, `Isolation Service`, `Servo Repository Contract`, `Servo Routes Tests`, `Repository Provider Functions`, `SSE Streaming`, `Relay E2E Tests`, `Servo Domain Exceptions`, `Servo State Store Tests`, `Angle-Counts Conversion`, `Calibration Service Tests`, `Servo Router Endpoints`, `Events & Composition Root`, `Settle State Reading`, `Dev Runner & Settings`?**
  _High betweenness centrality (0.037) - this node is a cross-community bridge._
- **Why does `wait_until()` connect `Motion/State Dependency Providers` to `Move Route Tests`, `Saved Position Exceptions`, `Simulated Servo Tests`, `Isolation Service`, `Repository Provider Functions`, `Servo Routes Tests`, `Relay E2E Tests`, `Servo Domain Exceptions`, `Servo State Store Tests`, `Diagnostics & Isolation Routes`, `Calibration Service Tests`, `Relay Error Paths`, `Bridge Relay Tests`, `Relay Failure Paths`, `Settle State Reading`, `Dev Runner & Settings`?**
  _High betweenness centrality (0.034) - this node is a cross-community bridge._
- **Why does `ServoStateStore` connect `Servo Repository Contract` to `Failed Read Guard Tests`, `Servo Domain Exceptions`, `Repository Provider Functions`, `Angle-Counts Conversion`, `SSE Streaming`, `Servo State Store Tests`, `Servo Routes Tests`, `Motion/State Dependency Providers`, `Saved Position Repository Contract`, `Domain Entities & Servo Abstraction`, `Servo Router Endpoints`, `Operator Event Log`, `Events & Composition Root`, `Motion Service`?**
...
- **Should `Bridge Network Relay (C++)` be split into smaller, more focused modules?**
