...
- **Speed ceiling, measured**: the servo saturates near 1100 counts/s regardless of the commanded value (~66 output deg/s). `default_speed_dps = 30` (fixed for every move, R9) sits well inside it. **D35** (closed 1 Sept 2026) confirmed the `GoalSpeed` conversion itself is accurate — see `skills/uno-q-st3215/SKILL.md` §2.
