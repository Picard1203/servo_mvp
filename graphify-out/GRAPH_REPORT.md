...
## Corpus Check
- 445 files · ~442,112 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 5561 nodes · 7362 edges · 455 communities (356 shown, 99 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 397 edges (avg confidence: 0.59)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `6f1f9567`
...
3. `ArduinoMCPServer` - 62 edges
4. `TelemetrySnapshot` - 56 edges
5. `wait_until()` - 49 edges
6. `Superpowers Release Notes` - 42 edges
7. `Database` - 40 edges
8. `ZeroReference` - 37 edges
9. `TEST()` - 37 edges
10. `get_isolation_service()` - 36 edges
...
## Communities (455 total, 99 thin omitted)
...
### Community 1 - "TelemetryService"
Cohesion: 0.05
Nodes (36): get_database(), get_telemetry_repository(), get_telemetry_service(), Returns the telemetry service. Also ticks the isolation reconciler and idle…, Returns the shared database wrapper. Returns: The process-wide database., Returns the telemetry repository. Returns: The process-wide telemetry…, Packs telemetry samples in range into a compact binary byte stream. Args:…, Persists the full sensory input every sampler interval. (+28 more)

### Community 2 - "test_sqlite_zero_repository.py"
Cohesion: 0.11
Nodes (12): fixture, SqliteZeroRepository: CRUD, active selection, datum upsert., Zero repository over a fresh database. Returns: The repository under test., Builds an unsaved zero entity. Args: name: Zero name. counts: Raw counts.…, Create, read, delete., Active-baseline selection., Upsert of THE calibration datum., repo() (+4 more)
...
Cohesion: 0.10
Nodes (13): SQLite connection management and schema initialization., SQLite implementation of the app-state key/value repository., Returns a stored value. Args: key: State key. Returns: The stored value, or…, Persists a value, replacing any previous one for the same key. Args: key: State…, Stores small operator-intent flags in the app_state table., SqliteAppStateRepository, fixture, SqliteAppStateRepository: get/set of persisted operator-intent flags. (+5 more)

### Community 10 - "Database"
Cohesion: 0.12
Nodes (10): Connection, Database, Adds columns introduced after a database was first created. Uses the ALTER-and-…, Returns the shared SQLite connection. Returns: The open connection., Closes the connection. The process-wide singleton never needs this in…, Creates tables and indexes when missing. Returns: None., Owns the SQLite connection and serializes all access to it. ``write_lock``…, Fresh-database schema. (+2 more)
...
### Community 15 - "get_settings"
Cohesion: 0.04
Nodes (56): BaseSettings, FastAPI, create_app(), FastAPI application assembly: routers and domain-error mapping. Construction of…, Creates and configures the FastAPI application. Returns: The configured…, Maps domain exceptions to HTTP responses. Args: app: The FastAPI application.…, _register_error_handlers(), get_settings() (+48 more)
...
Cohesion: 0.07
Nodes (19): Servo API routes: state, move, stop, lock, calibrate, recover., POST /api/v1/servo/recover., guard_move_to_lock surfaces as 409 reason=moving., An unreachable target must be refused, not silently clamped., Calibrating on a dead bus must refuse, not store a zero., A failed read yields no numbers at all, not a position alone. D16.…, Nulling a field must not remove it: clients read every key., D23, amends ADR-0008: the same rule as the five readings above. moving and the… (+11 more)
...
Cohesion: 0.09
Nodes (20): One persisted telemetry row. Attributes: timestamp: Unix timestamp of the…, TelemetrySample, Persists one sample. Args: sample: The sample to store. Returns: None., Yields samples inside a time range, oldest first. Args: ts_from: Range start,…, SQLite implementation of the telemetry repository., Stores telemetry samples in the telemetry table., Persists one sample. Args: sample: The sample to store. Returns: None., Counts samples in range and returns count and base timestamp. (+12 more)
...
### Community 22 - "_stream_generator"
Cohesion: 0.09
...
Cohesion: 0.06
Nodes (34): Closed items, D10 — `logger.exception` swallows the exception; the sampler's real fault was a thread-safety bug in the SQLite layer, D11 — A single failed poll is presented as a disconnection, D13 — Requests arriving faster than slots free up are refused, D14 — The most likely error in the system shows the operator "Failed to fetch", D15 — A command in flight looks identical to a command that did nothing, D16 — On a failed read the operator is shown 0.0 V, 0.0 A, 0.0 °C as if measured, D18 — A failed CSV export navigates the operator out of the application (+26 more)

### Community 30 - "MCPServiceImpl"
Cohesion: 0.18
...
Cohesion: 0.04
Nodes (48): get_app_state_repository(), get_event_service(), get_isolation_service(), get_motion_service(), get_state_store(), Returns the persisted operator-intent repository. Returns: The process-wide…, Returns the atomic servo/lock/baseline/isolation state store. Returns: The…, Returns the motion service. Returns: The process-wide motion service. (+40 more)

### Community 37 - "BridgeServoRepository"
Cohesion: 0.08
Nodes (19): BridgeServoRepository, Performs one real bus read. The caller must hold the lock. Returns: The…, Starts a move toward an absolute counts target. A new position command also…, Stops motion at the current position. Returns: None., Configures the servo's dead-zone width. Args: counts: Dead-zone width in…, Configures single-turn or multi-turn absolute positioning. Args: multi_turn:…, Cuts or restores drive torque while sensors stay powered (R2). Uses _call…, Reads register 0x28 directly (R2 board verification). Diagnostic only -… (+11 more)
...
### Community 48 - "ServoStateStore"
Cohesion: 0.03
Nodes (56): InvalidReadingError, Raised when an operation needs a reading the servo did not supply., get_servo_repository(), get_zero_repository(), Returns the servo repository chosen by use_hardware_servo. Simulated by default…, Returns the zero repository. Returns: The process-wide zero repository., Instantaneous sensory readout from the servo layer. Attributes: raw_counts:…, TelemetrySnapshot (+48 more)
...
### Community 53 - "deps.py"
Cohesion: 0.03
Nodes (108): delete, IsolationDep, MotionDep, Typed application settings loaded from the environment / .env file., In-memory ring buffer of structured events for the events endpoint. Kept…, get_zero_service(), Composition root: cached provider functions that construct and wire. This is…, Returns the zero service. Returns: The process-wide zero service. (+100 more)
...
### Community 127 - "mcp-tool-router.ts"
Cohesion: 0.14
...
Cohesion: 0.09
Nodes (12): Stops motion at the current position. Returns: None., Records the range configuration. The simulator already models unbounded signed…, Configures the simulated dead-zone width. Args: counts: Dead-zone width in…, Cuts or restores simulated drive torque (R2). Mirrors the real controller's un-…, Returns the simulated torque state (R2 board verification). Returns: 1 when…, Trips the simulated overload fault (testing/commissioning aid). Returns: None., Advances position toward the target until the process ends. Returns: None., Thread-driven simulation of one ST3215-class servo. (+4 more)
...
Cohesion: 0.09
Nodes (16): Event, EventService, One operator-facing event. Attributes: timestamp: ISO timestamp. event: Dotted…, Thread-safe fixed-size store of recent events., Stores one event. Args: event: Dotted event identifier. message: Human-readable…, Returns the newest events, newest first. Args: limit: Maximum number of events…, ABC, Abstract persistence of telemetry samples. (+8 more)
...
Cohesion: 0.07
Nodes (17): BridgeStub, Recording stub of the Arduino Bridge., Records a provided callback. Args: name: Bridge function name. fn: The…, Records a call and returns the configured result. Args: name: Bridge function…, System API routes: health and events., GET /api/v1/system/events., Health reporting when the board runtime is absent., The health endpoint names the servo backend in use. (+9 more)
...
Cohesion: 0.04
Nodes (70): ActiveZeroError, DatumZeroError, DomainError, IsolatedError, LockedAndIsolatedError, LockedError, MovingError, NotFoundError (+62 more)

### Community 446 - "Serial Monitor Issues"
Cohesion: 0.50
...
## Knowledge Gaps
- **2429 isolated node(s):** `state`, `REFUSALS`, `EVENT_LABELS`, `DAY_SHEET_COLS`, `RAW_HEADERS` (+2424 more)
...
- **Why does `TelemetrySnapshot` connect `ServoStateStore` to `TestMove`, `TelemetryService`, `get_state_store`, `BridgeServoRepository`, `SimulatedServoRepository`, `TestCalibrate`, `TestIsolate`, `test_servo_routes.py`, `TestTravelWindow`, `deps.py`, `test_motion_service.py`?**
  _High betweenness centrality (0.007) - this node is a cross-community bridge._
- **Why does `SimulatedServoRepository` connect `SimulatedServoRepository` to `ServoStateStore`, `deps.py`?**
  _High betweenness centrality (0.005) - this node is a cross-community bridge._
- **Are the 20 inferred relationships involving `ServoStateStore` (e.g. with `IsolationService` and `MotionService`) actually correct?**
  _`ServoStateStore` has 20 INFERRED edges - model-reasoned connections that need verification._
- **Are the 36 inferred relationships involving `TelemetrySnapshot` (e.g. with `ServoRepository` and `BridgeServoRepository`) actually correct?**
  _`TelemetrySnapshot` has 36 INFERRED edges - model-reasoned connections that need verification._
