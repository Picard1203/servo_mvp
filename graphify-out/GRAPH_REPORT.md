...
## Corpus Check
- 434 files · ~409,218 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 5198 nodes · 6595 edges · 464 communities (359 shown, 105 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 310 edges (avg confidence: 0.62)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `a00170ce`
...
## Communities (464 total, 105 thin omitted)
...
### Community 6 - "schemas/servo.py"
Cohesion: 0.09
Nodes (30): MotionDep, get_state(), post_lock(), post_move(), post_recover(), post_stop(), get, post (+22 more)

### Community 7 - "Database"
Cohesion: 0.17
Nodes (9): Database, Creates tables and indexes when missing. Returns: None., Adds columns introduced after a database was first created. Uses the ALTER-and-…, Owns the SQLite connection and serializes write access. SQLite permits…, Database: schema creation, migration of old schemas, row survival., Upgrading a database created before this change pack., Fresh-database schema., TestMigration (+1 more)
...
### Community 9 - "TelemetrySnapshot"
Cohesion: 0.07
Nodes (33): Exception, ActiveZeroError, DatumZeroError, DomainError, InvalidReadingError, MovingError, NotFoundError, Domain exceptions, mapped to HTTP responses by the application layer. (+25 more)
...
Cohesion: 0.08
Nodes (22): get_database(), get_telemetry_repository(), Returns the shared database wrapper. Returns: The process-wide database., Returns the telemetry repository. Returns: The process-wide telemetry…, Persists the full sensory input every sampler interval., Starts the background sampling thread. Returns: None., Streams a CSV of samples in the range, capped for the relay. Args: ts_from:…, TelemetryService (+14 more)
...
Cohesion: 0.03
Nodes (62): In-memory ring buffer of structured events for the events endpoint. Kept…, Composition root: cached provider functions that construct and wire. This is…, Immutable domain entities shared across layers., Coherent snapshot of servo + lock + baseline, read atomically. Attributes:…, A saved baseline position. Attributes: id: Database id; None before…, ServoStateView, ZeroReference, ABC (+54 more)
...
Cohesion: 0.05
Nodes (36): Logging configuration built on Logger461 (loguru JSON wrapper). Logger461 emits…, Initializes Logger461 for the process. Args: settings: Application settings…, setup_logging(), get_servo_repository(), get_state_store(), get_zero_repository(), Returns the atomic servo/lock/baseline state store. Returns: The process-wide…, Returns the servo repository chosen by use_hardware_servo. Simulated by default… (+28 more)
...
Cohesion: 0.06
Nodes (23): BaseSettings, Backend configuration, overridable via environment or .env. Attributes:…, Settings, McuLog, _now_iso(), Receives diagnostic events forwarded from the MCU side (backlog D3).…, Renames path to path + ".1" once it has grown past the threshold. A single…, Returns the current UTC time as an ISO-8601 string, millisecond precision.… (+15 more)
...
Cohesion: 0.21
Nodes (18): bin_t, Ack(), BridgeApi, BridgeApi::BridgeApi(), DrainDiagLog, FormatSnapshot, Register, FieldAt() (+10 more)
...
Cohesion: 0.16
Nodes (9): decode_sign_magnitude(), Decodes a sign-magnitude field from the servo wire format. STS position fields…, parametrize, The wire-format decoder stays available to callers., TestSignMagnitude, parametrize, Sign-magnitude decoding: the ~32700 wrap bug., Wire-format decoding for STS position fields. (+1 more)
...
### Community 302 - "TelemetrySample"
Cohesion: 0.17
Nodes (10): One persisted telemetry row. Attributes: timestamp: Unix timestamp of the…, TelemetrySample, Persists one sample. Args: sample: The sample to store. Returns: None., Yields samples inside a time range, oldest first. Args: ts_from: Range start,…, SqliteTelemetryRepository: add, ranged query, purge, fault columns., Builds a sample. Args: timestamp: Unix timestamp. overload: Overload flag…, Persistence round-trips., _sample() (+2 more)
...
Cohesion: 0.07
Nodes (20): BridgeStub, Recording stub of the Arduino Bridge., Records a provided callback. Args: name: Bridge function name. fn: The…, Records a call and returns the configured result. Args: name: Bridge function…, echo_server(), fixture, BridgeRelay: connection mirroring, byte pumping, teardown paths., Remaining failure branches. (+12 more)
...
### Community 395 - "test_servo_routes.py"
Cohesion: 0.06
Nodes (17): Servo API routes: state, move, stop, lock, calibrate, recover., POST /api/v1/servo/recover., guard_move_to_lock surfaces as 409 reason=moving., The +/-90 deg window the operators asked for., An unreachable target must be refused, not silently clamped., The refusal carries the configured step, not a hardcoded one. A regression…, POST /stop and /lock., POST /api/v1/servo/calibrate. (+9 more)
...
Cohesion: 0.05
Nodes (41): LockedError, OutOfTravelError, Raised when a target lies outside the servo's physical count range. The servo…, Raised when a commanded angle violates the configured step size., Raised when movement is requested while the digital lock is engaged., StepError, get_event_service(), get_motion_service() (+33 more)
...
### Community 448 - "test_relay_path.py"
Cohesion: 0.27
...
### Community 449 - "SqliteTelemetryRepository"
Cohesion: 0.17
...
### Community 457 - "TestNothingIsReportedAsMeasuredOnAFailedRead"
Cohesion: 0.47
...
- **Why does `http` connect `http` to `arduino-agent-bridge.js`, `server.cjs`, `mcp-server.ts`, `auth.test.js`, `server.test.js`, `branding.test.js`, `smoke-test.js`?**
  _High betweenness centrality (0.006) - this node is a cross-community bridge._
- **Why does `BridgeStub` connect `BridgeStub` to `test_relay_path.py`, `tests/conftest.py`, `BridgeServoRepository`, `decode_sign_magnitude`, `TestInvalidFlagHonoured`, `test_system_routes.py`, `test_mcu_log.py`, `test_bridge_servo_repository.py`, `TestCommands`, `TestResilience`, `TestFaultBits`, `TestSnapshotDecoding`?**
  _High betweenness centrality (0.004) - this node is a cross-community bridge._
- **Why does `BridgeServoRepository` connect `BridgeServoRepository` to `._command`, `TelemetrySnapshot`, `._read_uncached`, `decode_sign_magnitude`, `TestInvalidFlagHonoured`, `deps.py`, `test_bridge_servo_repository.py`, `TestCommands`, `TestResilience`, `TestFaultBits`, `get_state_store`, `TestSnapshotDecoding`?**
  _High betweenness centrality (0.004) - this node is a cross-community bridge._
