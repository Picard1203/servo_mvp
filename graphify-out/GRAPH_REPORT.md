...
## Corpus Check
- 150 files · ~135,308 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2347 nodes · 3982 edges · 166 communities (130 shown, 36 thin omitted)
- Extraction: 91% EXTRACTED · 9% INFERRED · 0% AMBIGUOUS · INFERRED: 369 edges (avg confidence: 0.55)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `9cfc0453`
...
1. `get_state_store()` - 81 edges
2. `ServoStateStore` - 64 edges
3. `wait_until()` - 53 edges
4. `TelemetrySnapshot` - 50 edges
5. `Closed items` - 47 edges
6. `Database` - 42 edges
7. `get_isolation_service()` - 37 edges
8. `TEST()` - 37 edges
...
Cohesion: 0.05
Nodes (41): NotFoundError, PositionOutOfRangeError, Raised when a referenced entity does not exist., Raised when a saved position's angle falls outside the travel window., Raised when an edit targets a saved position changed since it was read., StalePositionError, get_database(), get_saved_position_repository() (+33 more)
...
Cohesion: 0.05
Nodes (23): Records that the servo acknowledged an isolation write. Args: isolated (bool):…, Returns the acknowledged isolation state shown to operator. Returns: bool: True…, Marks the position reference as verified after calibration., Returns whether the position reference has been verified. Returns: bool: True…, Returns seconds left in the settle window (0.0 if none). Returns: float:…, Records a newly accepted move target and clears staleness. Args: target_deg…, Marks current target as no longer being pursued., Returns the current target and whether it is stale. Returns:… (+15 more)
...
### Community 6 - "TestRelayEndToEnd"
Cohesion: 0.29
Nodes (8): _http_request(), _parse(), Builds a raw HTTP/1.1 request as the shield's client would send. Args: path:…, Joins all net_tx chunks captured for a slot. Args: slot: Connection slot.…, Splits a raw HTTP reply into (status_code, json_body). Args: reply: Raw HTTP…, Requests through net_open/net_rx; replies through net_tx., _reply_bytes(), TestRelayEndToEnd
...
Cohesion: 0.13
Nodes (13): DuplicateNameError, Raised when a saved position name is already in use., Deletes one saved position. Args: position_id (int): Database identifier.…, Stores saved positions in the saved_positions table. Attributes: _db…, Maps a database row to the entity. Args: row (object): SQLite row. Returns:…, Persists a new saved position. Args: position (SavedPosition): Entity with…, Returns all saved positions, newest first. Returns: list[SavedPosition]: All…, Returns one saved position by id. Args: position_id (int): Database identifier.… (+5 more)

### Community 9 - "Database"
Cohesion: 0.08
Nodes (20): Database, Closes the connection., Owns the SQLite connection and serializes all access to it. Attributes:…, SQLite implementation of the app-state key/value repository., Returns a stored value. Args: key (str): State key to retrieve. Returns:…, Persists a value, replacing any previous one for the same key. Args: key (str):…, Stores small operator-intent flags in the app_state table. Attributes: _db…, SqliteAppStateRepository (+12 more)
...
### Community 17 - "TelemetryRepository"
Cohesion: 0.17
...
### Community 20 - "TestIsolationGate"
Cohesion: 0.20
...
### Community 23 - "ServoRepository"
Cohesion: 0.07
Nodes (27): InvalidReadingError, Raised when an operation needs a reading the servo did not supply., Calibration, The calibration datum. Attributes: raw_counts (int): Absolute encoder position…, AppStateRepository, ABC, Abstract persistence of small operator-intent flags that survive a reboot., Returns a stored value. Args: key (str): State key to retrieve. Returns:… (+19 more)
...
Cohesion: 0.28
Nodes (16): ClampAmplification(), ClampDeadband(), ClampMinStartForce(), ServoController, Begin, CentreHere, ClearFault, ConfigureRange (+8 more)
...
Cohesion: 0.06
Nodes (33): get_app_state_repository(), get_event_service(), get_isolation_service(), get_motion_service(), get_saved_position_service(), Returns the motion service. Returns: MotionService: The process-wide motion…, Returns the motor-isolation service. Returns: IsolationService: The process-…, Returns the saved-position service. Returns: SavedPositionService: The process-… (+25 more)

### Community 37 - "BridgeServoRepository"
Cohesion: 0.08
Nodes (19): BridgeServoRepository, Starts a move toward an absolute counts target. Args: target_counts (int):…, Stops motion at the current position. Returns: bool: True when the servo…, Configures the servo dead-zone width. Args: counts (int): Dead-zone width in…, Configures single-turn or multi-turn absolute positioning. Args: multi_turn…, Cuts or restores drive torque while sensors stay powered. Args: enabled (bool):…, Reads register 0x28 directly. Returns: Optional[int]: Register value (0 or 1),…, Invokes a Bridge function, converting failures into empty results. Args: name… (+11 more)
...
Cohesion: 0.08
Nodes (32): EventDep, ge, le, In-memory ring buffer of structured events for the events endpoint., get, StreamingResponse, SSE stream for servo state, saved positions and events., Streams servo state, saved positions, and events over SSE. Args: request… (+24 more)
...
Cohesion: 0.06
Nodes (58): CalibrationDep, FastAPI, IsolationDep, MotionDep, create_app(), FastAPI application assembly: routers and domain-error mapping., Creates and configures the FastAPI application. Returns: FastAPI: The…, Maps every domain exception to its HTTP response and log line. Args: app… (+50 more)
...
Cohesion: 0.09
Nodes (15): MotionService, Stops the current move at the present position. Raises:…, Changes the digital lock, honoring the optional move guard. Args: locked…, Clears a tripped overload fault by re-commanding the position. Raises:…, Records and raises a non-acknowledgement, never a false success. Args: event…, Decides whether the anti-backlash approach applies. Args: start_deg…, Validates and executes movement commands in output-degree space. Attributes:…, Runs the two-leg consistent-direction approach. Args: generation (int): Move… (+7 more)
...
Cohesion: 0.12
Nodes (14): SQLite connection management and schema initialization., Immutable domain entities shared across layers., A named, described position an operator can return to. Attributes: id…, SavedPosition, Persists a new saved position. Args: position (SavedPosition): Entity with…, Returns all saved positions, newest first. Returns: list[SavedPosition]: All…, Returns one saved position by id. Args: position_id (int): Database identifier.…, Overwrites a saved position's editable fields. Args: position_id (int):… (+6 more)

### Community 81 - "TestSamplerLifecycleIsolation"
Cohesion: 0.29
...
Cohesion: 0.09
Nodes (26): delete, patch, PositionsDep, create_position(), delete_position(), go_to_position(), list_positions(), get (+18 more)
...
Cohesion: 0.05
Nodes (26): get_state_store(), Returns the atomic servo/lock/baseline/isolation state store. Returns:…, An unreachable target must be refused, not silently clamped., TestOutOfTravelSurfaced, Calibration must not capture a reading the servo never gave. A failed read…, TestCalibrationRobustness, Target angle capture: set once on accept, stale (not cleared) on stop, never…, The overshoot leg commands PAST the requested angle - the operator must see… (+18 more)

### Community 132 - "SetSinks"
Cohesion: 0.50
Nodes (4): ByteSink, CloseSink, OpenSink, SetSinks

### Community 133 - "TelemetrySample"
Cohesion: 0.11
Nodes (16): get_telemetry_repository(), Returns the telemetry repository. Returns: TelemetryRepository: The process-…, One persisted telemetry row. Attributes: timestamp (float): Unix timestamp of…, TelemetrySample, Persists one sample. Args: sample (TelemetrySample): The sample to store., Yields samples inside a time range, oldest first. Args: ts_from (float): Range…, fixture, TelemetryService: sampling, CSV export, retention timing. (+8 more)

### Community 134 - "TelemetrySnapshot"
Cohesion: 0.11
Nodes (12): Instantaneous sensory readout from the servo layer. Attributes: raw_counts…, TelemetrySnapshot, Returns the full instantaneous sensory readout. Returns: TelemetrySnapshot:…, Simulated servo: sprint-1 stand-in for the real serial bus., Calibrating on a dead bus must refuse, not store a datum., The refusal carries the configured step, not a hardcoded one. A regression…, POST /stop and /lock., GET /api/v1/servo/state. (+4 more)
...
### Community 141 - "TestFineApproach"
Cohesion: 0.13
Nodes (9): _events(), Consistent-direction anti-backlash approach., A failed read must not crash the backlash decision - it has no information to…, The thread must never override a more recent command once it finally wakes up -…, The documented real risk on this filesystem (CLAUDE.md 6): SQLite cannot take…, D34: 0.06 deg is the real minimum step; a message rounded to 1 decimal cannot…, Forces the wait loop to stay alive for a fixed window, the same way the…, Returns recorded operator events. Args: backend: The backend fixture namespace.… (+1 more)
...
Cohesion: 0.05
Nodes (41): get_calibration_service(), get_servo_repository(), Returns the calibration service. Returns: CalibrationService: The process-wide…, Returns the servo repository chosen by use_hardware_servo. Returns:…, backend(), _clear_all_caches(), client(), fixture (+33 more)

### Community 397 - "deps.py"
Cohesion: 0.07
Nodes (42): BaseSettings, get_settings(), Typed application settings loaded from the environment / .env file., Backend configuration, overridable via environment or .env. Attributes:…, Returns the process-wide settings singleton. Returns: Settings: The cached…, Settings, Logging configuration built on Logger461 (loguru JSON wrapper)., Initializes Logger461 for the process. Args: settings (Settings): Application… (+34 more)
...
## Knowledge Gaps
- **366 isolated node(s):** `state`, `REFUSALS`, `EVENT_LABELS`, `DAY_SHEET_COLS`, `RAW_HEADERS` (+361 more)
...
- **Why does `ServoStateStore` connect `ServoStateStore` to `.snapshot`, `SavedPositionService`, `get_state_store`, `test_motion_service.py`, `TelemetryService`, `stream.py`, `TestDirection`, `deps.py`, `IsolationService`, `routers/servo.py`, `EventService`, `ServoRepository`, `MotionService`, `wait_until`?**
  _High betweenness centrality (0.031) - this node is a cross-community bridge._
- **Why does `get_state_store()` connect `get_state_store` to `ServoStateStore`, `get_isolation_service`, `TestRelayEndToEnd`, `stream.py`, `test_motion_service.py`, `TestDirection`, `TelemetrySample`, `TelemetryService`, `deps.py`, `TestFineApproach`, `TestSamplerLifecycleIsolation`, `routers/servo.py`, `wait_until`?**
  _High betweenness centrality (0.031) - this node is a cross-community bridge._
- **Why does `BridgeServoRepository` connect `BridgeServoRepository` to `TelemetrySnapshot`, `bridge_servo_repository.py`, `deps.py`, `BridgeStub`, `TestCommands`, `ServoRepository`, `test_bridge_servo_repository.py`, `wait_until`?**
